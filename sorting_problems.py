...
my_sorted_list = selection_sort(sort_me)
...
for key_pos in range(1, len(sort_me)):
    key_value = sort_me[key_pos]
    scan_pos = key_pos - 1  # look to the dancer on the left
    while (scan_pos >= 0) and (sort_me[scan_pos] > key_value):
        sort_me[scan_pos + 1] = sort_me[scan_pos]
        scan_pos -= 1

    # now everything is shifted to make room for the key_value
    sort_me[scan_pos + 1] = key_value
print(sort_me)
